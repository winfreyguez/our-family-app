...
function App() {
    ...
    return (
      <div style={{display:'flex', justifyContent:'center', alignItems:'center', height:'100vh', background:'#fff0f5'}}>
        <form style={{background:'white', padding:'2rem', borderRadius:'1rem', boxShadow:'0 4px 12px rgba(0,0,0,0.1)'}}>
          ...
          <button onClick={login} style={{width:'100%', background:'#f43f5e', color:'white', padding:'0.5rem', border:'none', borderRadius:'0.25rem', marginBottom:'0.5rem', cursor:'pointer'}}>Sign In</button>
          <button onClick={signUp} style={{width:'100%', background:'#e5e7eb', padding:'0.5rem', border:'none', borderRadius:'0.25rem', cursor:'pointer'}}>Create Account</button>
        </form>
      </div>
    )
  }
  ...
  return (
    <div style={{padding:'2rem', textAlign:'center'}}>
      ...
      <p>Your app is officially live!</p>
      <button onClick={() => supabase.auth.signOut()} style={{marginTop:'1rem', background:'#ef4444', color:'white', padding:'0.5rem 1rem', border:'none', borderRadius:'0.25rem', cursor:'pointer'}}>Log Out</button>
    </div>
  )
}
export default App
